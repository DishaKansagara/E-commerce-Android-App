                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            for (QueryDocumentSnapshot documentSnapshot : task.getResult()){
                                //Log.i("ERROR", documentSnapshot.get("icon").toString());
                        ...
                        if(task.isSuccessful()){
                            for (QueryDocumentSnapshot documentSnapshot : task.getResult()){
                                ...
                                if ((long)documentSnapshot.get("view_type") == 0){
                                    ...
                                    // 58
                                    long no_of_banners = Long.valueOf(documentSnapshot.get("no_of_banners").toString());
    ...
    public static void loadRatingList(final Context context){
        ...
        if (!ProductDetailsActivity.running_rating_query) {
            ...
            firebaseFirestore.collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_RATINGS")
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if (task.isSuccessful()) {
                        for (long x = 0; x < Long.valueOf(task.getResult().get("list_size").toString()); x++) {
...
//                     if (cartList.size() != 0){  TODO: unable to solve cart badge problem still have to figure out and change cart white name to cart wbalck in database
                                        ...
                                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                            if (task.isSuccessful()){
                                                for (QueryDocumentSnapshot documentSnapshot: task.getResult()) {
                                                    ...
                                                    }else if (documentSnapshot.get("type").toString().equals("Flat CAD * OFF") && lastSeenDate.before(documentSnapshot.getTimestamp("validity").toDate())){
                                                        rewardModelList.add(new RewardModel(documentSnapshot.getId(),documentSnapshot.get("type").toString()
                                                                , documentSnapshot.get("lower_limit").toString()
                                                                , documentSnapshot.get("upper_limit").toString()
